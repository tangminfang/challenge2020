...
def xgboost_stacking(x_train, y_train, x_test, n_folds=5):
    train_num, test_num = x_train.shape[0], x_test.shape[0]
    second_level_train_set = np.zeros((train_num,))
    second_level_test_set = np.zeros((test_num,))
    test_nfolds_sets = np.zeros((test_num, n_folds))
    ...
    for i,(train_index, test_index) in enumerate(kf.split(x_train)):
        ...
        dval = xgb.DMatrix(x_tst, label=y_tst)
        ...
        second_level_train_set[test_index] = bst.predict(dval)
        test_nfolds_sets[:,i] = bst.predict(dtest)
    second_level_test_set[:] = test_nfolds_sets.mean(axis=1)
    return second_level_train_set, second_level_test_set
...
for clf in [rf_model, adb_model, et_model, svc_model]:
    train_set, test_set = get_stacking(clf, train_x, train_y, test_x)
    train_sets.append(train_set)
    test_sets.append(test_set)

train_set, test_set = xgboost_stacking(train_x, train_y, test_x)
train_sets.append(train_set)
test_sets.append(test_set)
