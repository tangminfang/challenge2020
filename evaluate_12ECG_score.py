...
def evaluate_score(label_array, output_array,num_classes,thresold=0.5):
    ...
    for i in range(num_recordings):
        # jlb_locate = int(label_array[i])
        # jout_locate = int(output_array[i])
        output_line=output_array[i]
        for j in range(num_classes):
            if output_line[j]>thresold:
                output[i,j]=1
        # jlb_locate = label_array[i].astype(np.int)
        # jout_locate = output_array[i].astype(np.int)
...
def compute_beta_score(labels, output, beta, num_classes, check_errors=True):
    ...
    for j in range(num_classes):
        ...
        for i in range(num_recordings):
            ...
            if labels[i][j]==1 and output[i][j]==1:
                tp += 1/num_labels
            elif labels[i][j]==0 and output[i][j]==1:
                fp += 1/num_labels
            elif labels[i][j]==1 and output[i][j]==0:
                fn += 1/num_labels
            elif labels[i][j]==0 and output[i][j]==0:
                tn += 1/num_labels
